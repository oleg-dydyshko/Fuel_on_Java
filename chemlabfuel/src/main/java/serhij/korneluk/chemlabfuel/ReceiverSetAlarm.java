    ...
    private void Task() {
        new Thread(() -> {
            ...
            if (!g.equals("")) {
                ...
                ArrayList<ArrayList<Long>> notifikation = new ArrayList<>(gson.fromJson(notifi, type2));
                ...
                for (int i = 0; i < inventarny_spisok_data.size(); i++) {
                    if (notifikation.get(i).get(2) == 1L) {
                        long data;
                        ...
                        alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, mkTime(c.get(Calendar.YEAR), c.get(Calendar.MONTH), c.get(Calendar.DATE), c.get(Calendar.HOUR)), interV, pIntent);
                    }
                }
            }
        }).start();
